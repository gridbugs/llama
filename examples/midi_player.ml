...
let midi_signal (data : Llama_midi.Data.t) =
  ...
  let midi_event_signal = Midi.track_signal track clock in
  let sequencer_output = Midi.Sequencer.signal 40 midi_event_signal in
  let voices =
    List.map sequencer_output ~f:(fun { Midi.Sequencer.frequency_hz; gate } ->
